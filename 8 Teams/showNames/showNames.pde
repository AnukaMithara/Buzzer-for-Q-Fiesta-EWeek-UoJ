...
int[] buttonState = new int[8]; // array to store the state of the buttons
long[][] duration = new long[8][2]; // array to store the time duration              2 - duration   0- button name 3- pressed time
...
"KN/ VADDAKACHI CENTRAL COLLEGE", "MN/ SIDTHI VINAYAGAR HINDU COLLEGE", "School 7", "School 8"};
String schName;
...
void draw() {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        //Images
        if (buttonState[i] == 1) {
            image(img2,60 + i * 230, 50, 190, 190); 
        } else{      
            image(img1,60 + i * 230, 50, 190, 190); // draw the rectangle
        }
        
        //Numbers
        textSize(60);
        fill(255);
        text(i + 1, 155 + i * 230, 210); // draw the button number
    }
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (duration[i][0] > 0) {      
            ...
            textAlign(LEFT);
            textSize(40);
            text(schName, 300, 450 + y * 80);
            textAlign(RIGHT);
            textSize(50);
            text(minutes + " : " + seconds + " : " + milliseconds, 1750, 450 + y * 80);
            line(260, 470 + y * 80,1800,470 + y * 80);
            y++;
        } 
    }     
